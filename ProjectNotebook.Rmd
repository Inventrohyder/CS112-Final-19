...
paths <- synth.wdk2.paths("MLI", "Mali", 1980, 1989, 1990, 2008, 6, 8)  
...
bfaso.weights <- round(paths[[2]]$solution.w,2)
country.names <- tapply(afripanel$Country,afripanel$wbcode2,unique)
rows <- unname(sapply(rownames(bfaso.weights),function(x){country.names[names(country.names)%in%x]}))
bfaso.weights <- data.frame(rows,bfaso.weights)
bfaso.weights <-bfaso.weights[order(bfaso.weights[,2],decreasing=T),]
bfaso.weights <- bfaso.weights[which(bfaso.weights[,2]!=0),]
colnames(bfaso.weights) <- c("Contribution","Weights")
```
...
plot(impact) + 
  ggtitle("Causal Impact on Mali beyond 1991")
```
```{r}
impact <- show_impact_n('Mali', '1980', '2005', '1985')
plot(impact) + 
  ggtitle("Causal Impact on Mali beyond 1985")
```
```{r}
impact <- show_impact_n('Burkina Faso', '1980', '2005', '1990')
plot(impact) + 
  ggtitle("Causal Impact on Burkina Faso beyond 1990")
```
